...
public class ParametrizacaoResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new parametrizacao, or with status 400 (Bad Request) if the parametrizacao has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/parametrizacaos")
    @Timed
    public ResponseEntity<Parametrizacao> createParametrizacao(@RequestBody Parametrizacao parametrizacao) throws URISyntaxException {
        log.debug("REST request to save Parametrizacao : {}", parametrizacao);
        if (parametrizacao.getId() != null) {
            throw new BadRequestAlertException("A new parametrizacao cannot already have an ID", ENTITY_NAME, "idexists");
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated parametrizacao,
     * or with status 400 (Bad Request) if the parametrizacao is not valid,
     * or with status 500 (Internal Server Error) if the parametrizacao couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/parametrizacaos")
    @Timed
    public ResponseEntity<Parametrizacao> updateParametrizacao(@RequestBody Parametrizacao parametrizacao) throws URISyntaxException {
     ...
     *
     * @return the ResponseEntity with status 200 (OK) and the list of parametrizacaos in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the parametrizacao, or with status 404 (Not Found)
     */
